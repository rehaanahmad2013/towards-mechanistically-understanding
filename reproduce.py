...
def evaluate(model, tokenizer, chains, device, scan_samples, do_scan):
    ...
    print("PRELIMINARY_METRICS=" + json.dumps(result, sort_keys=True), flush=True)
    if not do_scan:
        return result
    ...
    for chain in chains[:scan_samples]:
        ...
        for anchor_ids in anchor_variants:
            try:
        ...
        if entity_positions is None:
            raise ValueError(f"Head entity {chain['head']!r} not found in tokenized prompt")
        random_positions = [1]
